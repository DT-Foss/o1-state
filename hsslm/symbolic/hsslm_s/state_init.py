...
def initialize_symbol_state(
    ...
    Parameters
    ----------
    vocab_size_or_symbols : int or Sequence[str]
        Either the number of symbols, or an ordered list of symbol strings.
        When a list is given, each symbol is hashed directly, so the same
        word always maps to the same state regardless of vocabulary ordering.
    state_dim : int
    ...
    """
    if isinstance(vocab_size_or_symbols, int):
        symbols = [f"__SYM_{i}__" for i in range(vocab_size_or_symbols)]
    else:
        symbols = list(vocab_size_or_symbols)

    vocab_size = len(symbols)
    state_matrix = np.zeros((vocab_size, state_dim + 1), dtype=float)

    for i, symbol_str in enumerate(symbols):
        # F9: Primitive state from hash of the actual symbol
